import {
  buildFashnTryOnMaxRunBody,
  type FashnTryOnMaxInput,
  type FashnTryOnMaxResult,
...
export async function runFashnTryOnMax(
  input: FashnTryOnMaxInput
): Promise<FashnTryOnMaxResult> {
  const promptUsed = input.prompt?.trim() ?? "";

  if (process.env.AI_MOCK_MODE !== "0") {
    return {
      ok: true,
      imageUrl: input.modelImageUrl,
      requestId: "mock-fashn-tryon-max",
      rawStatus: "completed",
      promptUsed,
    };
  }
  ...
  try {
    const run = await fashnFetch<FashnRunResponse>("/run", {
      method: "POST",
      body: JSON.stringify(buildFashnTryOnMaxRunBody(input)),
    });
    ...
    if (!requestId) {
      return {
        ok: false,
        errorCode: "FASHN_TRYON_MAX_NO_REQUEST_ID",
        errorMessage: "FASHN Try-On Max /run returned no prediction id",
      };
    }
      ...
      if (status.status === "completed") {
        ...
        if (!imageUrl) {
          return {
            ok: false,
            requestId,
            errorCode: "FASHN_TRYON_MAX_NO_OUTPUT",
            errorMessage: "FASHN Try-On Max completed but returned no output URL",
      ...
      if (status.status === "failed") {
        const err =
          typeof status.error === "object" && status.error !== null
            ...
            : String(status.error ?? "FASHN Try-On Max failed");
        return {
          ok: false,
          requestId,
    ...
    return {
      ok: false,
      requestId,
      errorCode: "FASHN_TRYON_MAX_TIMEOUT",
      errorMessage: `FASHN Try-On Max timed out after ${(MAX_POLL_ATTEMPTS * POLL_INTERVAL_MS) / 1000}s`,
    };
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Unknown FASHN Try-On Max error";
